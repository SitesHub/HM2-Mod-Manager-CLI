...
helpF_mouse_click() {
    ...
    read -n 1 -ep "Insert an option: " answer
...
fixPath() { 
    ...
    [[ $fixedPath =~ ^\.\. ]] && {
        ...
        normalizedPath_withDots=$(
            cd "$fixedPath" &>/dev/null && 
                pwd || 
                echo "Fail"
        )
        [[ $normalizedPath_withDots != "Fail" ]] &&
            fixedPath=${fixedPath/\.\./$normalizedPath_withDots} ||
            return 1
    ...
    [[ $fixedPath =~ ^\. ]] && 
        fixedPath=${fixedPath/\./$currentPath}
}
...
set_path() {
    ...
    showMenu() {  
        ...
        read -n 1 -ep "Choose an option: " answer
        ...
        [[ "$answer" = "1" ]] && {
            ...
            fixPath "$answer"
            ls "$fixedPath" &>/dev/null && gamePath "$fixedPath" || {
        ...
        [[ "$answer" = "2" ]] && {
            ...
            fixPath "$answer"
            ls "$fixedPath" &>/dev/null && backupPath "$fixedPath" || {
...
installation() {
    ...
    for file in */*.patchwad ; do
        ...
        # This condition is essential otherwise it might break things
        [[ $file == "*/*.patchwad" ]] && break
    	currentFolder="${file%/*}/ "
    ...
    [[ $patchwad_CurrentFolder != 0 ]] && 
        mv *.patchwad "$modsDirectoryFolder"/ && 
    ...
    [[ ${#folders[*]} != 0 ]] && {
        ...
        echo Installed the folder/s that contain/s patchwad files inside /mods
...
install() {
    ...
    showMenu() {
        ...
        [[ "$answer" == "2" ]] && {
            ...
            fixPath "$answer"
            ls "$fixedPath" &>/dev/null && fromArchive "$fixedPath" || {
        ...
        [[ "$answer" == "3" ]] && {
            ...
            read -ep "Insert the URL: " answer

            [[ "$answer" =~ (http|https):\/\/ ]] && fromURL "$answer" || {
                
                clear
                echo "$RED$answer$NORMAL is not a valid URL"
                echo
                read
                showMenu
...
uninstall() {
    ...
    checkIfClicked && clear
    originalPath=$(pwd)
    checkIfClicked && originalPath=$(dirname $0)
